...
export default function ShoppingCart() {
  ...
  return (
    <Box sx={{ backgroundColor: '#fff', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <Box sx={{ backgroundColor: '#f5f5f5', padding: isMobile ? '12px 16px' : '20px 50px', borderBottom: '1px solid #e0e0e0' }}>
      ...
      <Box sx={{ maxWidth: '1400px', width: '100%', margin: '0 auto', padding: isMobile ? '20px 16px' : '50px 50px', flex: 1 }}>
        <Typography variant="h1" sx={{ textAlign: 'center', marginBottom: isMobile ? '24px' : '50px', color: '#000', letterSpacing: '1px' }}>
          MY SHOPPING BAG
        </Typography>
        ...
        <Box sx={{ backgroundColor: '#fff', overflow: 'hidden', marginBottom: '40px' }}>
          ...
          {cartList.length === 0 ? (
            ...
            cartList.map((item) => {
              if (isMobile) {
                return (
                  <Box key={item.id} sx={{ padding: '16px', borderBottom: '1px solid #e0e0e0' }}>
                    <Box sx={{ display: 'flex', gap: '12px' }}>
                      <Box sx={{ width: '90px', height: '90px', backgroundColor: '#f5f5f5', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flexShrink: 0 }}>
                        {item.image ? (
                      ...
                      <Box sx={{ flex: 1, minWidth: 0 }}>
                        <Typography variant="h3" sx={{ marginBottom: '6px', color: '#000' }}>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: '10px', marginTop: '12px' }}>
                          ...
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: '0px', border: '1px solid #ddd' }}>
                            <IconButton 
                              onClick={() => handleDecrease(item)}
                              sx={{ width: '34px', height: '34px', backgroundColor: '#fff', fontSize: '18px', borderRight: '1px solid #ddd', fontWeight: '300' }}
                            >
                              −
                            </IconButton>
                            <Box sx={{ width: '40px', textAlign: 'center', fontSize: '15px', fontWeight: '400', height: '34px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                              {item.quantity}
                            </Box>
                            <IconButton 
                              onClick={() => handleIncrease(item)}
                              sx={{ width: '34px', height: '34px', backgroundColor: '#fff', fontSize: '18px', borderLeft: '1px solid #ddd', fontWeight: '300' }}
                            >
                              +
                            </IconButton>
                          </Box>
                          <IconButton 
                            ...
                            sx={{ fontSize: '22px', color: '#666', fontWeight: '300', marginLeft: 'auto' }}
                          >
                            ×
                          </IconButton>
                        </Box>
        ...
        <Box sx={{ display: isMobile ? 'block' : 'flex', justifyContent: 'space-between', alignItems: isMobile ? 'stretch' : 'flex-start', marginTop: '30px', gap: isMobile ? '16px' : '0px' }}>
          <NavLink to="/" style={{ textDecoration: 'none' }}>
            <Button sx={{ width: isMobile ? '100%' : undefined, backgroundColor: '#000', color: '#fff', padding: '16px 45px', fontSize: '13px', textTransform: 'uppercase', letterSpacing: '1px', fontWeight: '500' }}>
              CONTINUE SHOPPING
            </Button>
          </NavLink>

          <Box sx={{ backgroundColor: '#fff', padding: '0px', minWidth: isMobile ? '100%' : '450px' }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', marginTop: '10px',marginBottom: '20px', alignItems: 'center' }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: '15px' }}>
              ...
              <NavLink to="/checkout" style={{ flex: 1, textDecoration: 'none' }}>
